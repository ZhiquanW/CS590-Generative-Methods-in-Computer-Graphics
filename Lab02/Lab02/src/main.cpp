...
void DrawBezier(vector<Vect3d> control_points, int sample_num) {
  ...
  for (int i = 0; i <= sample_num; ++i) {
    Vect3d tmp_point = GetDeCasteljauPoint(degree, degree, (float)i * interval,
                                           control_points);
    DrawPoint( tmp_point, Vect3d(0.5f, 0.2f, 0.8f));
  }
}
...
void Lab01() {
  ...
  if (bezierFlag) {
    DrawBezier(bezier_control_vertex, steps);
  }
  if (pieceBezierFlag) {
    int counter = 0;
    for (int i = 0; i < 3; ++i) {
      ...
      counter--;
      DrawBezier(tmpControlPoitns, steps);
	}
  }
}
